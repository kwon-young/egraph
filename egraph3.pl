...
- lookup/2 (semidet, pure, steadfast): From canonical Nodes, obtain Id for Key. Prune by standard order, confirm with (==). Binds Id only. Pre: canonical via merge_nodes/2. Cost: O(N).
- add//2, add/4 (det, pure w.r.t. Keys): Build Key = F(ChildIds) left‑to‑right (stable arg order ⇒ congruence). Emit only Key‑Id; never unify Ids. Duplicates removed by merge_nodes/2. Pre: In is an ordset.
...
- merge_nodes//0, merge_nodes/2 (det, Id effects only): Canonicalize to one Key‑Id per Key: sort → group → unify each group’s Ids with its first; repeat to a fixpoint. Keys never unify. Terminates because the number of distinct Ids strictly decreases.
- merge_group/4 (det, Id effects only): For Key‑[H|T], unify all Ids in T with H; Changed=true iff T \= []. Representative is H.
- make_index/2 (det, pure): Build rbtree Id→[Keys] from canonical Nodes. The Id variable itself is the map key (by identity). Rebuild after aliasing. Cost: O(N log N). Keys stored as‑is.
- rules//3, rule//3 (nondet, pure): Apply each Rule(Node,Index)// to Node. May emit only Key‑Id items and (=)/2 between Ids; must not inspect or bind Ids. Output: per‑node, then per‑rule.
- match/4 (det, pure): Run Rules over Worklist using Index; produce scheduled Matches (nodes and (=)/2). No unification here. Output order: worklist, then per‑rule.
- push_back//1 (det, pure): Append a list to DCG output in O(1) via difference lists; scheduling only.
- rebuild//1 (det, Id effects only): Apply (=)/2 equalities (alias Ids), enqueue nodes, then canonicalize (merge_nodes//0).
...
- Id discipline (applies to all): Ids are fresh logic variables acting as mutable class identifiers. Alias via unification only; compare by identity (==), never by name/print‑name.
