...
%  Notes: Id unification can instantiate variables inside Keys; a subsequent pass may reveal duplicates.
...
%  Notes: BA is a fresh Id; AB=BA is consumed by rebuild//1 (the only place that unifies Ids).
comm((A+B)-AB, _Nodes) -->
   !,
   [B+A-BA, AB=BA].
comm(_, _) --> [].
...
%  Notes: Fresh Ids are introduced for AB/ABC_; equality items are deferred to rebuild//1.
...
%  Notes: Evaluation uses is/2; preserves the numeric type produced by is/2. Emits nodes/equalities only; unification is deferred to rebuild//1.
...
%  Notes: Uses number/1 to guard both sides; VB is considered in constant_folding_b//4.
...
%  Notes: Keep Rules pure; any unification must go through rebuild//1 via (=)/2 items.
...
%  Notes: rule//3 is invoked by rules//3; it does not inspect or modify Ids.
...
%  Notes: Uses transpose_pairs/2 and group_pairs_by_key/2. Id keys are logic variables.
make_index(In, Index) :-
...
%  Notes: DCG-only helper; does not inspect or touch Ids.
push_back(L), L --> [].
...
%  Notes: Simple alias for extract/2 that returns its input.
extract(Nodes) :-
   extract(Nodes, Nodes).
...
%  Notes: Uses member/2 on Keys and thus can alias Id variables; use only for validation and discard any aliases.
