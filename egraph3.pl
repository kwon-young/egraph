...
%  - In/Out are ordsets of Key-Id; prunes by standard order, confirms equality with (==) to preserve var identity.
...
%    - Ids are logic variables; never compare by name.
...
%  Determinism: det.
%  Note: Keys are never unified; only Id variables are.
