...
- lookup/2 (semidet, pure, steadfast): Lookup Id for Key in a canonical ordset. Prune by standard order; confirm identity with (==) to preserve variable identity. Binds only Id; no allocation on success; fails if absent. Pre: canonical ordset (from merge_nodes/2). O(N) worst case. Note: undefined on non-canonical input.
- add//2, add/4 (det, pure w.r.t. Keys): Build Key = F(ChildIds) left-to-right (stable arg order ⇒ congruence). Emit only Key-Id pairs; never unify Ids. Duplicates are removed later by merge_nodes/2. Uses foldl/5; steadfast. Pre: In is an ordset.
...
- merge_group/4 (det, logical effects): For Key-[H|T], unify each Id in T with H; Changed=true iff T ≠ []. Only Id vars unify; Keys never do.
- make_index/2 (det, pure): Build rbtree Id → [Keys] from canonical Nodes using the Id variable identity as the map key. Rebuild after any Id aliasing. O(N log N). Note: Keys are stored as-is; no unification here.
- rules//3, rule//3 (nondet, pure): Apply each DCG Rule(Node,Index)// to Node using Index. Rules may emit only Key-Id pairs and (=)/2; must not inspect or bind Ids. Output order: per-node, then per-rule; stable.
...
- rebuild//1 (det, logical effects): Apply (=)/2 equalities (alias Ids), enqueue Key-Id items, then canonicalize via merge_nodes//0. Only Id vars unify; Keys never do.
- unif/1 (semidet, impure by design): True for Eq = (A=B); performs A=B (no occurs-check). Intended only for rebuild//1 via exclude/3. Safe because Ids are fresh, acyclic logic vars. Note: do not call from rewrite rules.
- comm//2, assoc//2, assoc_//3, reduce//2, constant_folding//2, constant_folding_a//4, constant_folding_b//4 (nondet, pure): Example rewrite rules/helpers. Emit nodes/equalities only; no in-place rewrites; use Index to restrict search when possible. Must not inspect or bind Ids.
- extract/2, extract_node/1 (semidet, aliases Ids): Validation helpers used by extract//0. Intentionally alias Ids via member/2; validation only; discard bindings afterwards.
- saturate/4 (det, driver): Iterate with a step bound; rebuild the Id→Keys index each iteration; only Ids may unify (in rebuild/merge). Fixpoint is length-based; alias-only steps do not count as progress.
- DCG bridging: DCG nonterminals compile to extra args; DCG forms remain pure producers. merge_nodes//0 is a DCG shim over merge_nodes/2.
...
%  - Pre: Pairs was produced by merge_nodes/2 (canonical).
%  - Uses standard order to narrow the search, then (==) to confirm Key identity (preserves variable identity).
%  - Binds Id only; no allocation on the success path; fails if Key absent.
%  - Ids are fresh logic vars (mutable class ids); never compare by print-name.
%  Complexity: O(N). Semidet; steadfast; pure w.r.t. Keys/Ids.
%  Note: On non-canonical inputs behavior is undefined; canonicalize first (or use add_node/3).
...
%  - Emits only Key-Id pairs; never unifies Ids. Duplicates are removed by merge_nodes/2.
%  Pre: In is an ordset.
%  Cost: build O(|Term|), insert O(N). Det; steadfast; pure w.r.t. Keys.
%  Notes: Ids are fresh logic vars (class ids). DCG form is a pure producer.
add(Term, Id, In, Out) :-
   (  compound(Term)
   -> Term =.. [F | Args],
...
%  Pre: In is an ordset. Ids are logic vars (class ids).
%  Complexity: O(N). Det; quasi-pure (no Id unification).
%  Note: For non-canonical sets prefer add_node/3 or canonicalize later via merge_nodes/2.
...
%  Canonicalize to one Key-Id per Key; repeat until stable under Id aliasing.
%  - Algorithm: sort/2 → group_pairs_by_key/2 → unify group Ids with the first; repeat if any merge occurred.
%  - Effects: only Id vars unify; Keys never do. Unification may instantiate variables inside Keys; the next pass collapses collisions.
%  - Complexity: O(N log N) per pass; multiple passes possible. Det.
%  Termination: the number of distinct Id vars strictly decreases on any merge. Representative is the first Id after sorting (backtrackable).
%  Note: Representatives are not stable across runs/backtracking; never rely on their identities.
merge_nodes(In, Out) :-
   sort(In, Sort),
...
%  - Effects: only Id vars unify; Keys unchanged. Representative Id is H.
%  Complexity: O(|T|). Det.
%  Note: May instantiate variables embedded in Keys indirectly via Id aliasing.
...
%  Notes: Pure producer; steadfast; must not inspect or bind Ids. Nondet over Rules.
%  Note: Output order is stable: per-node, then per-rule; do not rely on representative Ids.
...
%  Notes: Pure producer; steadfast; must not inspect or bind Ids. Determinism follows Rule//2.
%  Note: Kept separate to use sequence//2 cleanly and document the calling convention.
...
%  Note: Matches are scheduled only; side effects happen later in rebuild//1.
...
%  Note: Local scheduling primitive only; it does not rebuild indexes.
push_back(L), L --> [].
...
%  Note: Always follow any Id aliasing with merge_nodes/2 to rebuild representatives; never include Key=Key equalities here.
rebuild(Matches) -->
...
%  - MaxSteps: integer >= 0. The atom 'inf' is only valid on systems where it compares with numbers; on SWI‑Prolog pass a large integer instead.
...
%  - Note: If MaxSteps='inf', the guard (N > 0) requires a Prolog where 'inf' compares with numbers; otherwise use a large integer (e.g., on SWI‑Prolog).
%  Determinism: det driver; nondet only from Rules.
%  Note: Progress metric is length only; pure aliasing steps are invisible to the stop test.
...
%  Note: This is the only place outside merge_nodes/2 where Ids are explicitly unified on purpose.
unif(A=B) :- A=B.

%! extract(-Nodes) is semidet.
%  Validate and return Nodes (aliases Ids).
%  - Aliases class Id variables via member/2; use for validation only.
%  - To inspect without aliasing, use the node list directly.
%  Ids: logic variables; discard any bindings produced here.
%  Determinism: semidet.
%  Note: This predicate intentionally aliases Ids; never use in normal rewriting.
extract(Nodes) :-
   extract(Nodes, Nodes).
...
%  Determinism: semidet.
%  Note: Fails only if the index is corrupted (should not happen with merge_nodes/2).
extract(Nodes, Nodes) :-
