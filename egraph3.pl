...
%  Note: Id unification can instantiate variables inside Keys; a subsequent pass may reveal duplicates.
...
%  Note: AB and ABC_ are fresh Ids; ABC=ABC_ is consumed by rebuild//1.
...
%  Note: This is a simple alias for extract/2 that returns its input.
extract(Nodes) :-
   extract(Nodes, Nodes).
