...
- lookup/2 (semidet, pure, steadfast): On canonical Nodes, find Id for Key by standard-order pruning then (==) confirmation. Binds Id only; never allocates or unifies Ids. Pre: canonical (merge_nodes/2). Cost: O(N).
- add//2, add/4 (det, pure w.r.t. Keys): Build Key=F(ChildIds) left-to-right (stable arg order ⇒ congruence), emit only Key-Id. No Id unification; duplicates removed by merge_nodes/2. Pre: In is an ordset.
...
- make_index/2 (det, pure): Build rbtree Id->[Keys] from canonical Nodes; Id variables are used as map keys by identity. Rebuild after aliasing. Cost: O(N log N).
- rules//3, rule//3 (nondet, pure): Apply each Rule(Node,Index)// to Node. May emit only Key-Id items and (=)/2 between Ids; must not inspect/bind Ids. Output: per-node, then per-rule.
...
- rebuild//1 (det, Id effects only): Apply (=)/2 equalities (alias Ids), enqueue new nodes, then canonicalize (merge_nodes).
...
- DCG bridging: DCG calls to merge_nodes//0 expand to merge_nodes/2; no separate wrapper is required on SWI-Prolog. Nonterminals must remain pure producers.
