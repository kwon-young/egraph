...
- lookup/2 (semidet, pure, steadfast): Read-only Id lookup in a canonical ordset (exactly one Key-Id per Key). Prunes by standard order; confirms identity with (==) to preserve variable identity. Binds only Id; fails if Key is absent. No allocation; never unifies Keys or Ids. Pre: canonical ordset.
- add//2, add/4 (det, pure w.r.t. Keys): Build Key = F(ChildIds) left-to-right (stable arg order ⇒ congruence). Emit only Key-Id pairs (no (=)/2); never unify Ids; duplicates removed later by merge_nodes/2.
...
- merge_nodes//0, merge_nodes/2 (det, logical effects): Canonicalize to one Key-Id per Key via sort→group→unify-first. Repeat until stable (Id aliasing can instantiate variables inside Keys). Only Id vars unify; effects are backtrackable. Termination: number of distinct Id vars strictly decreases.
...
- rules//3, rule//3 (nondet, pure): Apply each DCG Rule(Node,Index)// to Node. Rules may emit only Key-Id pairs and (=)/2; must not inspect or bind Ids.
...
- rebuild//1 (det, logical effects): Apply (=)/2 equalities (alias Ids), enqueue resulting items, then canonicalize with merge_nodes//0. Only Id variables unify. Note: aliasing can instantiate variables embedded in Keys.
