...
- lookup/2 (semidet, pure, steadfast): Return Id for Key from a canonical ordset. Prunes by standard order; confirms Key identity with (==). Binds only Id; fails if absent. Pre: canonical set (merge_nodes/2). O(N).
- add//2, add/4 (det, pure w.r.t. Keys): Build Key = F(ChildIds) left-to-right (stable arg order ⇒ congruence) and emit Key-Id. Never binds Ids; duplicates removed by merge_nodes/2. Pre: In is an ordset.
- add_node/4, add_node/3 (det, quasi-pure): Ensure Node has a class Id. Reuse if present; otherwise insert Node-Id with a fresh logic variable. No canonicalization or Id unification.
- merge_nodes//0, merge_nodes/2 (det, logical effects): Canonicalize to one Key-Id per Key: sort → group → unify all group Ids with the representative; repeat while merges occur. Only Ids unify; Keys never do. Terminates because the number of distinct Ids strictly decreases.
- merge_group/4 (det, logical effects): For Key-[H|T], unify each Id in T with H; Changed=true iff T≠[].
- make_index/2 (det, pure): Build rbtree Id→[Keys] from canonical Nodes. The rbtree key is the Id variable itself. Rebuild after any Id aliasing. O(N log N). Keys stored as-is.
- rules//3, rule//3 (nondet, pure): Apply DCG rule Rule(Node,Index)// to Node using Index. Rules may emit only Key-Id pairs and (=)/2 equalities; must not inspect or bind Ids. Output order: per-node, then per-rule.
- match/4 (det, pure): Run Rules over a worklist using Index; collect nodes and equalities. No unification here. Output order follows worklist, then per-rule.
- push_back//1 (det, pure): Append a list to DCG output in O(1) via difference lists; scheduling only.
- rebuild//1 (det, logical effects): Perform (=)/2 (alias Ids), enqueue Key-Id items, then canonicalize (merge_nodes//0). Only Ids unify; Keys never do.
...
- DCG bridging: DCG nonterminals expand to extra arguments; DCG forms are pure producers. merge_nodes//0 is a shim for merge_nodes/2.
